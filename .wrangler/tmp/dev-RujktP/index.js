...
// .wrangler/tmp/bundle-CLbrbl/checked-fetch.js
...
var require_browser = __commonJS({
  "node_modules/pino/browser.js"(exports, module) {
    ...
    function set(self2, opts, rootLogger2, level) {
      Object.defineProperty(self2, level, {
        value: levelToValue(self2.level, rootLogger2) > levelToValue(level, rootLogger2) ? noop : rootLogger2[baseLogFunctionSymbol][level],
        writable: true,
        enumerable: true,
        configurable: true
      });
      ...
      self2[level] = createWrap(self2, opts, rootLogger2, level);
    ...
    function createWrap(self2, opts, rootLogger2, level) {
      return /* @__PURE__ */ function(write) {
        return /* @__PURE__ */ __name(function LOG() {
          ...
          if (opts.transmit) {
            ...
            const transmitValue = rootLogger2.levels.values[transmitLevel];
            const methodValue = rootLogger2.levels.values[level];
            ...
            transmit(this, {
              ...
              methodValue,
              transmitLevel,
              transmitValue: rootLogger2.levels.values[opts.transmit.level || self2._level],
              send: opts.transmit.send,
              val: levelToValue(self2._level, rootLogger2)
            }, args);
          }
        }, "LOG");
    ...
    function asObject(logger2, level, args, ts, formatters = {}) {
      const {
        level: levelFormatter = /* @__PURE__ */ __name(() => logger2.levels.values[level], "levelFormatter"),
...
// .wrangler/tmp/bundle-CLbrbl/middleware-loader.entry.ts
init_checked_fetch();

// .wrangler/tmp/bundle-CLbrbl/middleware-insertion-facade.js
init_checked_fetch();
...
var ProviderFailureError = class extends Error {
  static {
    __name(this, "ProviderFailureError");
  }
  originalError;
...
var SimpleCache = class {
  ...
  set(key, data, ttlSeconds) {
    this.cache.set(key, {
      data,
      timestamp: Date.now() + ttlSeconds * 1e3
    });
  }
  getTimestamp(key) {
    const item = this.cache.get(key);
...
var LiqwidGraphQLService = class {
  static {
    __name(this, "LiqwidGraphQLService");
  }
  config;
  cache;
  constructor(config) {
   ...
   */
  async getBasicLoans(addresses, options = {}) {
    ...
    try {
      const response = await this.executeGraphQL(LIQWID_QUERIES.BASIC_LOANS, { addresses });
      const loans = this.transformBasicLoans(response.liqwid.data.loans.results);
      this.cache.set(cacheKey, loans, 60);
      return loans;
    } catch (error) {
      throw new ProviderFailureError("Liqwid", `Failed to fetch basic loans for addresses: ${addresses.join(", ")}`, error);
    }
  }
  /**
   * Get detailed loan data for comprehensive monitoring
   */
  async getDetailedLoans(addresses, options = {}) {
    this.validateAddresses(addresses);
    ...
    try {
      const response = await this.executeGraphQL(LIQWID_QUERIES.DETAILED_LOANS, { addresses });
      const loans = this.transformDetailedLoans(response.liqwid.data.loans.results);
      this.cache.set(cacheKey, loans, 120);
      return loans;
    } catch (error) {
      throw new ProviderFailureError("Liqwid", `Failed to fetch detailed loans for addresses: ${addresses.join(", ")}`, error);
    }
  }
  /**
   * Get loans for the monitored payment address
   */
  async getMonitoredLoans(options = {}) {
    const addresses = [this.config.PAYMENT_ADDRESS];
    if (options.detailed) {
      return this.getDetailedLoans(addresses, options);
    } else {
      return this.getBasicLoans(addresses, options);
    }
  }
  /**
   ...
   */
  transformBasicLoans(loans) {
    return loans.map((loan) => ({
      id: loan.id,
      userAddress: "",
      // Not available in basic response
   ...
   */
  transformDetailedLoans(loans) {
    return loans.map((loan) => {
      const collaterals = loan.collaterals.map((col) => ({
        id: col.id,
      ...
      const collateralValue = collaterals.reduce((sum, col) => sum + col.qTokenAmount * col.exchangeRate, 0);
      return {
        id: loan.id,
        userAddress: "",
   ...
   */
  isCacheValid(data, maxAgeSeconds = 60) {
    if (!data)
      return false;
    return true;
  }
...
// dist/lib/healthFactor.js
init_checked_fetch();

// dist/lib/alerts.js
init_checked_fetch();
async function sendAlert(level, message, env) {
  if (!env.TELEGRAM_TOKEN || !env.CHATID) {
    console.error("Missing required Telegram configuration");
    return false;
  }
  const formattedMessage = `[${(/* @__PURE__ */ new Date()).toISOString()}] ${message}`;
  const MAX_RETRIES = 3;
  let retries = 0;
  while (retries <= MAX_RETRIES) {
    try {
      ...
      if (retries < MAX_RETRIES) {
        const backoffTime = Math.pow(2, retries) * 1e3;
...
var envSchema = external_exports.object({
  ...
  LIQWID_GRAPHQL_URL: external_exports.string().url(),
  // Health Factor thresholds
  HF_WARN: external_exports.coerce.number().positive(),
  HF_CRIT: external_exports.coerce.number().positive(),
  // Telegram notification settings (must be strings for CloudflareEnv compatibility)
  TELEGRAM_TOKEN: external_exports.string().min(1),
  CHATID: external_exports.string().min(1),
  // Cardano payment address to monitor
  PAYMENT_ADDRESS: external_exports.string().default(""),
  // Optional KV namespace for storing monitor state
  HF_MONITOR_STATE: external_exports.any().optional()
});
...
var logger = import_pino.default.default(loggerConfig);
var ContextLogger = class _ContextLogger {
  static {
    __name(this, "ContextLogger");
  }
  baseLogger;
  context;
   ...
   */
  withContext(context) {
    return new _ContextLogger(this.baseLogger, { ...this.context, ...context });
  }
  /**
...
var rootLogger = new ContextLogger(logger);
...
var dist_default = {
  ...
  async scheduled(event, env, ctx) {
    const logger2 = rootLogger.withContext({
      executionId: crypto.randomUUID(),
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    });
    logger2.info("Health Factor Monitor started");
    try {
      const validatedEnv = createEnv(env);
      const liqwidService = new LiqwidGraphQLService({
        LIQWID_GRAPHQL_URL: validatedEnv.LIQWID_GRAPHQL_URL,
        USER_AGENT: "CardanoDefiHelper/1.0",
        PAYMENT_ADDRESS: validatedEnv.PAYMENT_ADDRESS
        // Use the address from environment
      });
      logger2.info("Fetching loans from Liqwid");
      const loans = await liqwidService.getMonitoredLoans();
      logger2.info(`Fetched ${loans.length} loans from Liqwid`);
      const alertsSent = [];
      for (const loan of loans) {
        const healthFactor = loan.healthFactor;
        const assetSymbol = loan.borrowedAsset;
        if (healthFactor < validatedEnv.HF_CRIT) {
          logger2.warn(`Critical health factor detected: ${loan.id}`, { healthFactor, assetSymbol, threshold: validatedEnv.HF_CRIT });
          await sendAlert("critical", `\u{1F6A8} CRITICAL: ${assetSymbol} loan has health factor ${healthFactor.toFixed(2)}`, validatedEnv);
          alertsSent.push({ assetSymbol, id: loan.id, hf: healthFactor, level: "critical" });
        } else if (healthFactor < validatedEnv.HF_WARN) {
          logger2.warn(`Warning health factor detected: ${loan.id}`, { healthFactor, assetSymbol, threshold: validatedEnv.HF_WARN });
      ...
      logger2.info("Health Factor Monitor completed", {
        checked: loans.length,
        alertsSent: alertsSent.length,
        alerts: alertsSent
      });
    } catch (error) {
      logger2.error("Health Factor Monitor failed", {
        error: error instanceof Error ? error.message : String(error),
        stack: error instanceof Error ? error.stack : void 0
      });
      try {
        const lastErrorTime = env.HF_MONITOR_STATE ? await env.HF_MONITOR_STATE.get("lastErrorAlertTime") : null;
        const currentTime = Date.now();
        const errorMessage = error instanceof Error ? error.message : "Unknown error";
        const ERROR_ALERT_COOLDOWN_MS = 12 * 60 * 60 * 1e3;
        if (!lastErrorTime || currentTime - parseInt(lastErrorTime) > ERROR_ALERT_COOLDOWN_MS) {
          const validatedErrorEnv = createEnv(env);
          await sendAlert("error", `\u{1F525} ERROR: Health Factor Monitor failed: ${errorMessage}`, validatedErrorEnv);
          if (env.HF_MONITOR_STATE) {
            await env.HF_MONITOR_STATE.put("lastErrorAlertTime", currentTime.toString());
          }
          logger2.info("Error alert sent and cooldown timer started");
        } else {
          logger2.info("Error alert skipped due to rate limiting", {
            lastAlertTime: new Date(parseInt(lastErrorTime)).toISOString(),
            cooldownMs: ERROR_ALERT_COOLDOWN_MS
          });
        }
      } catch (alertError) {
        logger2.error("Failed to send error alert", {
